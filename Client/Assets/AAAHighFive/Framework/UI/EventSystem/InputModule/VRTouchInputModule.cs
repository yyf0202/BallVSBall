...
namespace HighFive
{
    public class VRTouchInputModule : PointerInputModule
    {
        ...
        private void FakeTouches()
        {
            ...
            if (leftPressData.buttonData != null && leftPressData.buttonData.pointerCurrentRaycast.gameObject != null)
            {
                //Debug.Log (LayerMask.LayerToName(leftPressData.buttonData.pointerCurrentRaycast.gameObject.layer));
                if (!(hitMask == (hitMask | (1 << leftPressData.buttonData.pointerCurrentRaycast.gameObject.layer))))
                {
                    return;
                }
            }
        ...
        private void ProcessTouchEvents()
        {
            for (int i = 0; i < Input.touchCount; ++i)
            {
                ...
                {
                    Debug.Log(LayerMask.LayerToName(pointer.pointerCurrentRaycast.gameObject.layer));
                    if (!(hitMask == (hitMask | (1 << pointer.pointerCurrentRaycast.gameObject.layer))))
                    {
                        RaycastResult temprr = new RaycastResult();
                        temprr.gameObject = null;
                        pointer.pointerCurrentRaycast = temprr;
                        released = false;
                        pressed = false;
                        return;
                    }
