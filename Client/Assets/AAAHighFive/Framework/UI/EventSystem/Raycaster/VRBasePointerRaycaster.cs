...
namespace HighFive
{
    /// This script provides shared functionality used by all VR raycasters.
    public abstract class VRBasePointerRaycaster : BaseRaycaster
    {
        ...
        public float MaxPointerDistance
        {
            get
            {
                if (VRPointerManager.Pointer == null)
                {
                    return 0.0f;
                }

                return VRPointerManager.Pointer.GetMaxPointerDistance();
            }
        }
        ...
        public float PointerRadius
        {
            get
            {
                ...
                VRPointerManager.Pointer.GetPointerRadius(out enterRadius, out exitRadius);
                if (VRPointerManager.Pointer.ShouldUseExitRadiusForRaycast)
                {
                    return exitRadius;
                }
                else
        ...
        protected VRBasePointerRaycaster()
        {
        }

        /// Returns true if the pointer and the pointer's transform are both
        /// available through the VRPointerManager.
        public bool IsPointerAvailable()
        {
            ...
            if (VRPointerManager.Pointer.GetPointerTransform() == null)
            {
                return false;
            }
        ...
        protected Ray GetRay()
        {
            ...
            Transform pointerTransform = VRPointerManager.Pointer.GetPointerTransform();
